...
- **动画模式替换** — 移除 3D 翻书动画（与 epub.js 内部页面过渡冲突），新增闪烁聚焦动画（blur 4px + opacity 0.5 → 聚焦清晰），柔和对焦效果
- **媒体键翻页** — 支持 Mac Touch Bar ◀▸ 按钮和蓝牙键盘媒体键翻页（前进/后退），设置页新增开关，默认开启
- **启动行为设置** — 设置页新增「启动行为」项，支持「书架」（默认）和「继续阅读」两种模式；选择继续阅读时自动打开上次阅读的图书
- **高亮笔记输入** — 添加文本标注时弹出输入框（`window.prompt`），支持为每个标注附加可选笔记

### Bug 修复

- **字体选择不生效** — CSS 选择器 `body` 优先级不足以覆盖 epub.js 内容文档中的元素样式；改为 `body, body *` 通用选择器提升优先级，使字体/行高/边距等布局属性正确应用于所有正文元素
- **深色模式切换章节后字体颜色反转丢失** — epubjs `themes.inject()` 仅检查 `theme.rules` 和 `theme.url`，忽略 `registerCss` 设置的 `serialized` CSS；在 content hook 注入布局样式后手动调用 `rendition.themes.select(themeRef.current)` 重新注入

## [1.4.3] — 2026-05-20

### 新特性

- **翻页动画选择** — Aa 面板新增翻页动画选择器，支持淡入淡出/左右滑动/闪烁聚焦/滑动+淡出四种效果；低性能模式开关可强制使用简单淡入淡出动画

## [1.4.2] — 2026-05-20

### 新特性

- **翻页滑动动画** — 阅读页翻页时新增左右滑动效果（前进从右滑入，后退从左滑入），200ms ease-out 过渡
