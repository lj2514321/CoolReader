...
import { bindReaderDocumentEvents, getReaderRelativeBounds } from '../../utils/readerContentEvents'
...
export function useBookEngine(shared: SharedRefs, opts: {
  ...
  const { applyLayout, saveReadingTime, saveBookReadingTime, setCurrentCfi, setBookmarks, setHighlights, setSelectionInfo } = opts
  ...
  const openBook = useCallback(async (filePath: string) => {
    const saveResults = await Promise.allSettled([saveReadingTime(), saveBookReadingTime()])
    saveResults.forEach(result => {
      if (result.status === 'rejected') logger.warn('[openBook] save previous reading session failed', result.reason)
    })
    if (sessionStartRef.current) {
      todaySecondsRef.current += Math.floor((Date.now() - sessionStartRef.current) / 1000)
      sessionStartRef.current = 0
    }
    if (bookSessionStartRef.current) {
      bookTodayRef.current += Math.floor((Date.now() - bookSessionStartRef.current) / 1000)
      bookSessionStartRef.current = 0
    }
    bookTodayRef.current = 0
    bookPathRef.current = ''
    ...
    if (!viewer) throw new Error('Reader view is unavailable')
    viewer.innerHTML = ''
    ...
    try {
      const [layoutSetting, themeSetting, customThemeSetting] = await Promise.all([
        loadSetting('readerLayout'),
        loadSetting('readerTheme'),
        loadSetting('customTheme'),
      ])
      if (layoutSetting) {
        const savedLayout = JSON.parse(layoutSetting) as ReaderLayout
        layoutRef.current = savedLayout
        setLayoutState(savedLayout)
      }
      if (customThemeSetting) {
        const savedCustomTheme = JSON.parse(customThemeSetting) as CustomTheme
        customThemeRef.current = savedCustomTheme
        setCustomThemeState(savedCustomTheme)
      }
      if (themeSetting && ['light', 'dark', 'sepia', 'custom'].includes(themeSetting)) {
        themeRef.current = themeSetting as ThemeMode
        setThemeState(themeSetting as ThemeMode)
      }
    } catch (e) {
      logger.warn('[openBook] load reader settings failed', e)
    }
    ...
    if (format === 'txt' || format === 'mobi') {
      ...
      syncRef.current = () => {
        ...
        sectionHrefRef.current = `${loc.chapterIdx}:0`
      ...
      // TXT content bubbles to the viewer. MOBI rebinds its callback after each iframe rewrite.
      ...
      if (format === 'txt') selectionTarget.addEventListener('mouseup', handleSelection)
      // Store for cleanup
    ...
    if (!hookRegistered.current) {
      ...
      rendition.hooks.content.register((view) => {
        const doc = (view as unknown as { document?: Document }).document
        ...
        doc.addEventListener('mouseup', () => {
          setTimeout(() => {
            ...
            try {
              const cfiRange = rendition.getCfiFromRange(range)
              const bounds = getReaderRelativeBounds(doc, range.getBoundingClientRect())
              setSelectionInfo({ text: text.slice(0, 200), cfiRange, bounds })
            } catch (e) {
              logger.warn('[selection] getCfiFromRange failed', e)
            }
          }, 0)
        })
      })
    ...
    const sync = () => {
      ...
      const pct = calculateSectionProgress(idx, count, runtimeLocation.start.displayed, runtimeLocation.atEnd)
      progressRef.current = pct
      cfiRef.current = cur.start.cfi || ''
      indexRef.current = idx
      const spineItems = book.spine?.items as Array<{ href?: string }> | undefined
    ...
    try {
      saved = await loadProgress(filePath)
    } catch (e) { logger.warn('[openBook] restore progress failed', e) }
    ...
    const adapterToc = (items: NavItem[]): import('../../adapters/BookAdapter').TocItem[] => items.map(item => ({
      label: item.label,
      location: item.href,
      subitems: item.subitems ? adapterToc(item.subitems) : undefined,
    }))
    epubAdapter.setBook(book, rendition, adapterToc(mapped), filePath, layoutRef.current, t, customThemeRef.current)
    ...
    try {
      ...
      for (const hl of hls) {
        try {
          await epubAdapter.addHighlight({ location: hl.location || hl.cfiRange, text: hl.text, color: hl.color })
        } catch (e) { logger.warn('[openBook] restore highlight failed', e) }
      }
    } catch (e) { logger.warn('[openBook] load bookmarks/highlights failed', e) }
  }, [readFile, saveReadingTime, saveBookReadingTime, applyLayout, setCurrentCfi, setSelectionInfo, setBookmarks, setHighlights])
  ...
  const setCustomTheme = useCallback((ct: CustomTheme) => {
    ...
    saveSetting('readerTheme', 'custom')
    const adapter = adapterRef.current
    if (adapter) {
      try {
  ...
  return {
    meta, toc, theme, customTheme, progress, layout,
